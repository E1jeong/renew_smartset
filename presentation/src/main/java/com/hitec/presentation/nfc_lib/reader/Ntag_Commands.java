...
public class Ntag_Commands {
     ...
     * @throws FormatException
     */
    public void write(byte[] data, byte blockNr)
            throws IOException, FormatException {
        // no answer
        answer = new byte[0];
        command = new byte[6];
        ...
        command[4] = data[2];
        command[5] = data[3];
        bLog.i_hex("NTAG_CMD", "write - 01", command, 6);
        nfca.transceive(command);
    }
     ...
     *
     * @param blockNr Block Number to begin Read
    ...
    public byte[] read(byte blockNr) throws IOException, FormatException {
        ...
        command[0] = (byte) 0x30;
        command[1] = blockNr;
        bLog.i_hex("NTAG_CMD", "read - 01", command, 2);
        answer = nfca.transceive(command);
        return answer;
    }
     ...
     *
     * @param pwd Password to authenticate with
    ...
    public byte[] pwdAuth(byte[] pwd) throws IOException {
        ...
        command[3] = pwd[2];
        command[4] = pwd[3];
        bLog.i_hex("NTAG_CMD", "pwdAuth - 01", command, 5);
        answer = nfca.transceive(command);
        return answer;
    }
