...
public class PeriodMeterReport extends NfcRxMessage {
    ...
    private int[] m_stUtility = new int[NfcConstant.MAX_METER_PER_TERMINAL]; //종류
    private int[] m_stMeterType = new int[NfcConstant.MAX_METER_PER_TERMINAL]; //계량기 코드
    private int[] m_stMeterValuePoint = new int[NfcConstant.MAX_METER_PER_TERMINAL]; //자릿수
    ...
    public int GetMeterConnection(int nPos, int nData) {
        if (nData >= MAX_NUM_OF_DATA) return NfcConstant.METER_VALID_VALUE_ERROR;
        ...
        else return NfcConstant.METER_VALID_VALUE_ERROR;
    }
    ...
    private void initArrayMember() {
        for (int i = 0; i < MAX_NUM_OF_DATA; i++) {
            ...
            m_stMeterConnection0[i] = NfcConstant.METER_VALID_OK;
            m_stMeterConnection1[i] = NfcConstant.METER_VALID_OK;
            m_stMeterConnection2[i] = NfcConstant.METER_VALID_OK;
    ...
    private void ParserMeterValue(
        ...
        if (fFirstValid == false) {
            m_nMeterConnection = NfcConstant.METER_VALID_ERROR;
    ...
    private int parserMeterDefault(
        ...
        if (Meter.CheckDecValid(pBuff, nOffSet, 4) == false) {
            m_nMeterConnection = NfcConstant.METER_VALID_VALUE_ERROR;
            m_strFirstMeterValue = "";
            m_strSecondMeterValue = "";
