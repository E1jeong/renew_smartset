...
public class ServerConnectReport extends NfcRxMessage {
    ...
    public boolean parse(byte[] rxdata) {
        ...
        if (m_nNodeMsgVersion == 0) {
            if (GetHeaderDevCode() == DEVICE_CODE_LORA) {
                m_nModemSnr = getHexData(m_nOffset++);
                if (m_nModemSnr > 0x80) {
                    m_nModemSnr -= 256;
                }
            } else {
                m_nModemBer = getHexData(m_nOffset++);
            }
