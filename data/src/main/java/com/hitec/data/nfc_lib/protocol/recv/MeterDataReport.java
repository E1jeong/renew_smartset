...
public class MeterDataReport extends NfcRxMessage {
    ...
    private int[] m_nUtility = new int[MAX_METER_PER_TERMINAL]; //종류
    private int[] m_nMeterType = new int[MAX_METER_PER_TERMINAL]; //계량기 코드
    private int[] m_nMeterPort = new int[MAX_METER_PER_TERMINAL]; //계량기 Port
    ...
    private String[] m_strMeterSn = new String[MAX_METER_PER_TERMINAL]; //일련번호
    private int[] m_nMeterValuePoint = new int[MAX_METER_PER_TERMINAL];
    private int[] m_nMeterCaliberCd = new int[MAX_METER_PER_TERMINAL]; //구경코드
    private String[] m_strMeterCaliber = new String[MAX_METER_PER_TERMINAL]; //구경정보
    private String[] m_strMeterVal = new String[MAX_METER_PER_TERMINAL]; //검침값
    private String[] m_strCaloriFlowVal = new String[MAX_METER_PER_TERMINAL]; //검침값
    private int[] m_nMeterBatt = new int[MAX_METER_PER_TERMINAL]; //계량기 배터리
    private int[] m_nMeterStatus = new int[MAX_METER_PER_TERMINAL]; //계량기 상태 플래그
    private boolean[] m_fMeterLeak = new boolean[MAX_METER_PER_TERMINAL]; //옥내누수
    ...
    public boolean GetSubTermValid() {
        if (m_nSubTermValid == METER_VALID_SUB_ERROR) {
            return false;
        }
        return true;
    }
    ...
    public boolean parse(byte[] rxdata) {
        ...
        if (m_nMeterCount == 0 || m_nMeterCount > MAX_METER_PER_TERMINAL) {
    ...
    private void parserMeterDefault(
        ...
        if (Meter.CheckDecValid(pBuff, nOffSet, 4) == true) {
            ...
            m_nMeterValid[nIdx] = METER_VALID_VALUE_ERROR;
            m_strMeterVal[nIdx] = "";
        }
        nOffSet += 4;
    ...
    private void InitMeterValueInfo(int nIdx) {
        m_nMeterValid[nIdx] = METER_VALID_OK;
    ...
    private int registerMeter(byte[] pBuff, int nOffSet, int nIdx) {
        ...
        if (m_nMeterDataFlag[nIdx] == 0x01) {
            m_nMeterValid[nIdx] = METER_VALID_ERROR;
            ...
            m_nMeterValid[nIdx] = METER_VALID_SUB_ERROR;
            m_nSubTermValid = METER_VALID_SUB_ERROR;
            nOffSet += 10; //이후 데이터는 0x00이므로 사용하지 않음
            if (
                    nMeterType == Meter.g_emMeterCaloriType.eOneTLDigital ||
                            nMeterType == Meter.g_emMeterCaloriType.eIcmDplc
