...
import com.hitec.data.nfc_lib.crypto.CRC32Calculator;
...
import com.hitec.data.nfc_lib.reader.I2C_Enabled_Commands.Access_Offset;
import com.hitec.data.nfc_lib.reader.I2C_Enabled_Commands.PT_I2C_Offset;
import com.hitec.data.nfc_lib.reader.Ntag_Auth.AuthStatus;
import com.hitec.data.nfc_lib.reader.Ntag_Get_Version.Prod;
import com.hitec.data.nfc_lib.reader.Ntag_I2C_Commands.Register;
import com.hitec.data.nfc_lib.util.ConstNfc;
...
public class Ntag_I2C_Connect implements ConstNfc {
    ...
    public Ntag_I2C_Connect(Tag tag, final Activity main, final byte[] passwd, final int authStatus) {
        try {
            Log.i("NTAG_DEMO", "Ntag_I2C_Demo - 00");
            if (tag == null) {
                this.main = null;
                this.tag = null;
                return;
            ...
            if (reader == null) {
                String message =
                        "The Tag could not be identified or this NFC device does not " +
            ...
            if (!prod.equals(Ntag_Get_Version.Prod.Unknown)) {
                if (
                        prod.equals(Ntag_Get_Version.Prod.NTAG_I2C_1k_Plus) ||
    ...
    //
    //			// wait to give the 쨉C time to reset the Tag
    //			Thread.sleep(50);
    //
    ...
    private Ntag_I2C_Plus_Registers getPlusAuth_Settings(
        ...
        //Access Register
        if (
                (0x0000080 & accessRegister[0]) >> Access_Offset.NFC_PROT.getValue() == 1
        ) {
        ...
        if (
                (
                        0x0000020 & accessRegister[0]
                ) >> Access_Offset.NFC_DIS_SEC1.getValue() ==
                        1
        ) {
        ...
        //PT I2C Register
        if (
                (0x0000008 & pti2cRegister[0]) >> PT_I2C_Offset.K2_PROT.getValue() == 1
        ) {
        ...
        if (
                (0x0000004 & pti2cRegister[0]) >> PT_I2C_Offset.SRAM_PROT.getValue() == 1
        ) {
    ...
    public String NfcReadNdef() {
        try {
            ...
            Log.i(
                    "NTAG_DEMO",
                    "NfcReadNdef : message:" + message + " bytes:" + bytes
            );
    ...
    public boolean NfcWriteNdef(String writeText) {
        try {
            ...
            Log.i(
                    "NTAG_DEMO",
                    "NfcWriteNdef : writeText:" + writeText + " bytes:" + bytes
            );
            return true;
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public boolean NfcDataTransive(
        ...
        int i, j;
        try {
            Log.i("NTAG_DEMO", "NfcDataTransive : respWaitTime:" +
                    respWaitTime + " startWaitTime:" + startWaitTime);
    ...
    public boolean NfcPeriodDataTransive(
        ...
        try {
            Log.i(
                    "NTAG_DEMO",
                    "NfcPeriodDataTransive : respWaitTime:" +
                            respWaitTime +
                            " startWaitTime:" +
                            startWaitTime
            );
            ...
            for (int nTime = 0; nTime < waitTime; nTime += defaultReadTime) {
                try {
                    ...
                    Log.i(
                            "NTAG_DEMO",
                            "NfcPeriodDataTransive => Exception : readSRAMBlock"
                    );
                    e.printStackTrace();
                    exceptionCnt++;
                    if (exceptionCnt > 10) {
                ...
                //Log.i("NTAG_DEMO", "NfcPeriodDataTransive dataRx.length:" + dataRx.length);
                Log.i(
                        "NTAG_DEMO",
                        "NfcPeriodDataTransive Wait:" + nTime + " = waitTime:" + waitTime
                );

                if (dataRx.length > 10) {
                    devCode = (int) (dataRx[0] & 0xFF);
                    msgType = (int) (dataRx[10] & 0xFF);
                }
                ...
                if (
                    ...
                    if (msgType == NfcConstant.NODE_RECV_PERIOD_METER_REPORT) {
                        ...
                        Log.i(
                                "NTAG_DEMO",
                                "NfcPeriodDataTransive totalBlock:" +
                                        totalBlock +
                                        " currBlock:" +
                                        currBlock
                        );
                        ...
                        Log.i(
                                "NTAG_DEMO",
                                "NfcFlashDataTransive totalBlock:" +
                                        totalBlock +
                                        " currBlock:" +
                                        currBlock
                        );
            ...
            Log.i(
                    "NTAG_DEMO",
                    "NfcPeriodDataTransive => CommandNotSupportedException"
            );
            e.printStackTrace();
        } catch (Exception e) {
        ...
        try {
            if (authStatus == AuthStatus.Unprotected.getValue()) {
                ...
                reader.unprotectPlus();
                    ...
                    authStatus == AuthStatus.Protected_W.getValue() ||
