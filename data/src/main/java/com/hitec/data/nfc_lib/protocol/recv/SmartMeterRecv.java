...
public class SmartMeterRecv extends NfcRxMessage {
    ...
    public boolean parse(byte[] rxdata) {
        ...
        if (r_checksum != c_checksum) {
            ...
            } else if (m_nMeterMsgType == NfcConstant.METER_RECV_CONF_REPORT) {
                parserConfReport(hexData, m_nOffset);
            } else if (m_nMeterMsgType == NfcConstant.METER_RECV_CONF_METER_REPORT) {
                parserConfMeterReport(hexData, m_nOffset);
            } else if (m_nMeterMsgType == NfcConstant.METER_RECV_CERTI_COMP_REPORT) {
                parserCertiCompReport(hexData, m_nOffset);
            } else if (
                    m_nMeterMsgType == NfcConstant.METER_RECV_VALVE_CONTROL_REPORT
            ) {
                parserMeterValveControlReport(hexData, m_nOffset);
            } else if (m_nMeterMsgType == NfcConstant.METER_RECV_CONF_AUTO_REPORT) {
    ...
    private void parserConfReport(byte[] pBuff, int nOffSet) {
        if (m_nDataLength == CONF_ULTRA_LEN) {
            m_nMeterFlowType = NfcConstant.METER_FLOW_TYPE_ULTRA;
        } else {
            m_nMeterFlowType = NfcConstant.METER_FLOW_TYPE_DIGITAL;
        }
        ...
        if (m_nMeterFlowType == NfcConstant.METER_FLOW_TYPE_ULTRA) {
